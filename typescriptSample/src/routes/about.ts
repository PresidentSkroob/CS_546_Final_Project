...
router.use('/', async (req, res) => {
  try {
    ...
    for (let x=0; x < foundUsers.length; x++ ) {
      {if (foundUsers[x].level == "admin") {
        let salonistName = foundUsers[x].firstName + " " + foundUsers[x].lastName;
        let prof = foundUsers[x].job.charAt(0).toUpperCase() + foundUsers[x].job.slice(1);
        let biography = foundUsers[x].biography;
        
        let reviewByHaidresser = await reviews.getAllReviewsByHairdresserId2(foundUsers[x]._id!);
